---
tags: ["tutorial", "component"]
---

# Tags

A [front matter](https://github.com/jxson/front-matter) attribute allows us to define tags per file.

## Example

import TagPills from "@/components/TagPills";

<TagPills />

## Usage

You can use any valid front matter format for the tags attribute, but please follow these
conventions for consistency:

import { Tabs } from "nextra/components";

<Tabs items={['Multiple tags', 'Single tag', 'No tags']}>
  <Tabs.Tab>
    ```md filename="multiple-tags.md"
    ---
    tags: ["tag-1", "tag-2"]
    ---

    # My new article

    This is an example article with tags.
    ```

  </Tabs.Tab>
  <Tabs.Tab>
    ```md filename="single-tag.md"
    ---
    tags: "tag-1"
    ---

    # My new article

    This is an example article with a tag.
    ```

  </Tabs.Tab>
  <Tabs.Tab>
    ```md filename="no-tags.md"
    # My new article

    This is an example article with no tags.
    ```

  </Tabs.Tab>
</Tabs>
