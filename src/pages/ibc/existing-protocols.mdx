...
informed of its newfound wealth. To solve this problem, the [ADR-8 specification][adr-8]
...
type-safe way to generate the complete `IbcMsg::Transfer`, the other is a helper type
...
- `ibc_source_callback`, receiving an `IbcSourceCallbackMsg` enum which can be one of
  two types:
...
For the `IbcMsg::Transfer` message, a success acknowledgement means that the tokens
were successfully transferred to the destination chain. It
