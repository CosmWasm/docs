...
it is not very useful. Let's make it a bit reactive.

First, we need to add [`serde`](https://crates.io/crates/serde) crate to our dependencies. It would
...
```rust {7-10, 22-29} copy filename="src/lib.rs" template="empty"
use cosmwasm_std::{
...
significant difference is a type of `deps` argument. For `instantiate`, it was a `DepMut`, but here
...
checking the message `sender`). It is not a case for queries. Queries are supposed just purely to
...
contracts in ways different than querying them (which is handled by the `deps` argument). The query
...
[`Binary`](https://docs.rs/cosmwasm-std/latest/cosmwasm_std/struct.Binary.html) type using the
[`to_json_binary`](https://docs.rs/cosmwasm-std/latest/cosmwasm_std/fn.to_json_binary.html)
function.

## Improving the message

We have a query entry point, but there is a problem with the query message. It is always an empty
...
```rust {11-14, 27, 30-38} copy filename="src/lib.rs" template="empty"
use cosmwasm_std::{
...
serialize to a JSON with a single field - the name of the field will be an enum variant (in our
case - always "greet" - at least for now), and the value of this field would be an object assigned
...
massive unreadable `match` statement. To make the code more
[SOLID](https://en.wikipedia.org/wiki/SOLID), we will refactor it and take out handling the `greet`
message to a separate function.

```rust {31, 35-45} copy filename="src/lib.rs" template="empty"
use cosmwasm_std::{
    entry_point, to_json_binary, Binary, Deps, DepsMut, Empty, Env, MessageInfo, Response, StdResult,
};
use serde::{Deserialize, Serialize};
...
query-response type `GreetResp` as I may have different responses for different queries. I want the
...
Also, you might pass `deps` and `env` arguments to all your query handlers for consistency. I'm not
...
```rust copy filename="src/msg.rs" template="empty"
use serde::{Deserialize, Serialize};
...
You probably noticed that I made my `GreetResp` fields public. It is because they have to be now
accessed from a different module. Now move forward to the `src/contract.rs` file:

```rust {1} copy filename="src/contract.rs"
...
from my `query` function in `src/contract.rs`. I will have a function with this attribute. Still, I
wanted to split functions' responsibility further - not the `contract::query` function is the
