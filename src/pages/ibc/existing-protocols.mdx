...
pub fn ibc_destination_callback(
    ...
    // Let's parse the packet data to get that information:

    /// This is the data format for a v1 transfer packet (v2 looks different)
    /// See <https://github.com/cosmos/ibc/tree/main/spec/app/ics-020-fungible-token-transfer#data-structures>
    #[cw_serde]
    struct TransferPacketData {
        denom: String,
        amount: Uint256,
        /// The sender on the source chain
        /// This is not of type `Addr` because it is not a valid address on this chain
        sender: String,
    ...
    ensure_eq!(
        packet_data.denom,
        "untrn", // assuming this runs on Neutron,
        StdError::generic_err("only want to handle this chain's native token")
    );

    // ... do something with our tokens here
...
sure that the receiver of the funds is the contract that receives the callbacks.
