...
On real-life blockchain based on Cosmos-SDK, user addresses are derived from the public key of the
...
- finally the resulting RIPEMD-160 hash is then encoded using Bech32 format; a human-readable prefix
  (specific to the blockchain, e.g., cosmos for the Cosmos Hub) is prepended.

In **`MultiTest`**, user addresses are indeed derived from a user-provided string, such as an alias
(e.g., "alice", "bob", "creator", "owner", etc.), and not from an actual public/private key pair,
which simplifies address creation for testing purposes.
