...
cases where **`MultiTest`** does not have a default implementation for the feature, users may
provide their own, using `AppBuilder`'s function listed in **AppBuilder constructor** column. Names
of **`MultiTest`** feature flags required to enable specific functionality are provided in the
column **Feature flag**.
