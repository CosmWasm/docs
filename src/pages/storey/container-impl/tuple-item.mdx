...
The `phantom` field allows us to use the type parameters `L` and `R` without actually storing any values of
that type.
...
The `TupleItemAccess` struct is our accessor. It's a facade that's used to actually access the data in
...
Okay! We can build more complex data structures by composing built-in containers and our custom ones.
Pretty cool, right?
...
<Callout type="info">
  ...
  If you're planning to publish your abstractions somewhere, this is **definitely** a good idea.
  We didn't do it in the examples to try and keep complexity down.
</Callout>
