...
In the previous section we talked about the [`execute` endpoint]. The `query` endpoint is actually pretty
similar to its sibling `execute`, but with one key difference: The storage is only accessible immutably.
