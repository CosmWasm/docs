...
This example smart contract is a simple **counter** that allows users to initialize the counter,
...
- The counter can be initialized with a value of zero or a specific non-negative integer in the
  range of 0 to 255.
- Users can increment, decrement, or set the counter to a new value within the range of 0 to 255.
- Boundary conditions are checked, preventing the counter from going below zero or beyond 255.
...
the Sylvia framework, the following examples will guide you through the essential content of smart
contract, in this case, the **counter**.

<Callout type="info" emoji="🦘">
  Click on the card below to jump into the details of the **counter** smart contract.
</Callout>
