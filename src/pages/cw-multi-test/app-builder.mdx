...
following code snippet. After initialization, [`App{:rust}`][App] provides two methods to access
the storage: [`storage{:rust}`][storage] and [`storage_mut{:rust}`][storage_mut]. The former is
used for reading the storage, while the latter allows both reading and writing.
...
[`with_storage{:rust}`][with_storage] method of [`AppBuilder{:rust}`][AppBuilder].
The example below achieves the same result as the previous one.
