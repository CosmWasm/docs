...
<Callout type="warning">
  Please note that the following example assumes an ICS20 v1 channel. At the
  time of writing, the specification and implementation have just been extended
  with a v2 which changes the packet format. If you want to use this in
  production code, you should make sure to support both formats, such that a
  channel upgrade does not break your contract.
</Callout>
