...
Use `sv::messages` to expand your contract functionality with the Sylvia
interface.

<Callout>
  The `sv::messages` attribute expects that the trait is implemented on the
  contract type.
</Callout>
...
In most cases, the only parameter the `sv::messages` expects is a path to the
