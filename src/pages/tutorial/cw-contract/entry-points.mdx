...
- [`info: MessageInfo`](https://docs.rs/cosmwasm-std/latest/cosmwasm_std/struct.MessageInfo.html) contains
  metainformation about the message that triggered the execution - an address that sends the
...
- [`msg: Empty`](https://docs.rs/cosmwasm-std/latest/cosmwasm_std/struct.Empty.html) is the message
  triggering execution itself - for now, it is the `Empty` type that represents `{}` JSON, but the type
...
[`#[entry_point]`](https://docs.rs/cosmwasm-std/latest/cosmwasm_std/attr.entry_point.html). Its purpose
is to wrap the whole entry point to the form Wasm runtime understands. proper Wasm entry points
