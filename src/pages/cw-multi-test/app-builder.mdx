...
always finalize the chain-building process by calling the [`build{:rust}`][build] method with a
chain initialization callback function. If no specific chain initialization is required, you can use
the provided [`no_init`](app#no_init) callback. Otherwise, you can initialize the chain using a
custom callback function. An example of how to initialize a user's balance is shown below.
