...
besides Rust. Go to the `src/contract.rs` and add a test in its module:
...
module which contains local function unit tests. The problem is - this test doesn't build yet. We
need to tweak our message types a bit.
...
[`Clone`](https://doc.rust-lang.org/std/clone/trait.Clone.html) is not needed yet, but it is also
...
our `query` function. Let's update our test:
